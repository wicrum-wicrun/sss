/-  *sss
::
|=  [pub=(list [mark path]) sub=(list [mark path]) agent=hoon]
=>
  |%
  +$  state
  ...
  ++  build-agent  |^
    |=  byk=beak
    %-  slap  :_  [%ktls agent-bunt agent]
    %+  reel  (weld pub sub)  ::TODO delete duplicates
    |=  [=mark * result=_!>([custom-types ..zuse)]
    =/  lake  .^(vase %ca /(scot %p p.byk)/[q.byk]/(scot r.byk)/lak/[mark]/hoon)
    (slop lake(p [%face mark p.vase]) result)
    ::
    ++  custom-types  ^?
      |%
      +$  card  (wind note gift)
    ...
    ++  agent-bunt
      ...
      :+  %$  ~
      %-  my  ^-  (list [term hoon])
      :~  :-  %on-init   !,  *hoon                    *(quip card _^|(..on-init)
          :-  %on-save   !,  *hoon                    *vase
          :-  %on-load   !,  *hoon  |~  vase          *(quip card _^|(..on-init)
          :-  %on-poke   !,  *hoon  |~  [mark vase]   *(quip card _^|(..on-init)
          :-  %on-watch  !,  *hoon  |~  path          *(quip card _^|(..on-init)
          :-  %on-leave  !,  *hoon  |~  path          *(quip card _^|(..on-init)
          :-  %on-peek   !,  *hoon  |~  path          *(unit (unit cage)))
          :-  %on-fail   !,  *hoon  |~  [term tang]   *(quip card _^|(..on-init) 
          :-  %on-agent  !,  *hoon  |~  [wire sign:agent:gall]
                                    *(quip card _^|(..on-init)
          :-  %on-arvo   !,  *hoon  |~  [wire sign-arvo]
                                    *(quip card _^|(..on-init)
          :-  %on-rock   :+  %brsg  on-rock-sample
                         !,  *hoon  *(quip card _^|(..on-init)
      ==
      ::
      ++  agent-sample
        :-  %bccl
        :-  +:!,(*hoon ,bowl:gall)
        %+  weld
          %-  drop  %-  mole  |.  ::  using +corl find-errors :(
          :-  %bccl  ;;  (lest spec)
          %+  turn  pub
          |=  [=mark *]
          :+  %make  wing/%map
      ...
      ++  on-rock-sample
        ...
        ::
            ...
            |=  [=mark =path]
            :-  %bccl
            :~  bccl/(reel path |=([=term res=_~[base/%null]] [leaf/tas/term res]))
                like/[~[%rock] ~[~[mark]]]
                make/[wing/%unit like/[~[%wave] ~[~[mark]]]]
        ==  ==
      --
    --
  --
